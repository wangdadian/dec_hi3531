...
long CChannelManager::OnOsdChange(int chnno, void* pData, void* param)
{
    ...
    memcpy(&(pThis->m_pChannelConfig->m_encsettings.encset[chnno].osd), \
        pOsd, sizeof(EncOsdSettings));
...
void CChannelManager::SaveUrl( int nChannelNo, char *szUrl, int nFPS )
{
	...
	printf( "---------------save url %d %s %d\n", nChannelNo, szUrl, nFPS );
	char szOrigUrl[256]={0};
	int nOrigFps = 0;
	...
	{
		printf("the same url, dont need save, return.\n");
								...
								EncNetParam encnet )
{
	...
	if ( nChannelNo < 0 || nChannelNo >= g_nMaxEncChn ) return;
	memcpy( &m_pChannelConfig->m_encsettings.encset[nChannelNo].net.net[encnet.iIndex], &encnet, 
		sizeof(encnet));
	if ( m_pEncoder[nChannelNo] != NULL )
	{
	    if(encnet.iEnable == 1)
		...
		m_pEncSet->SetEncTransfer( &(m_pChannelConfig->m_encsettings.encset[nChannelNo].net), 
			nChannelNo, false   );
	}
}
								...
								EncNetParam encnet )
{
	...
	for ( int i = 0; i < g_nMaxEncChn; i++ )
	{
		...
		if ( strcmp( m_pChannelConfig->m_encsettings.encset[i].szDeviceId, szDeviceId ) != 0 ) continue;
		if ( nStreamNo > 0 ) i++;
		memcpy( &m_pChannelConfig->m_encsettings.encset[i].net.net[0], &encnet, 
		sizeof(encnet));
...
void CChannelManager::OpenEnc( int nChannelNo )
{
	pthread_mutex_lock(&m_openlock);
