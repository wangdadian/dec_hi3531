...
long CVmux6671Protocol::OnGetEncStreamParam(ENCODERPARAM *param)
{
    ...
    // 
    param->iEncodeType = 5;// H264
    param->iFrameRate = sEncParam.iFPS;
    param->iCodeRate = 1000*(sEncParam.iBitRate);
    param->iEncodePolicy = 1;// 码率优先
    param->iEncodeWay = 1;
    param->bStreamFlag = 1; // 码流开关，固定打开
    param->bReserve = 0; // 保留
    param->uiGOP = sEncParam.iGop;
    param->uiGOP_M = 0;// GOP组合方式
    param->uiBasicQP = sEncParam.iMinQP; // 基础QP
    param->uiMaxQP = sEncParam.iMaxQP;
    param->uiCodeRateType = sEncParam.iCbr;
    param->uiMaxCodeRate = (sEncParam.iBitRate+2000)*1000;
    param->uiMinCodeRate = (sEncParam.iBitRate-500)*1000;
...
long CVmux6671Protocol::OnStartEncoder(STREAMCTL *param)
{
    ...
    _DEBUG_("##Vmux667## Start Encoder Video Transfer");
...
long CVmux6671Protocol::OnStopEncoder(STREAMCTL *param)
{
    ...
    _DEBUG_("##Vmux667## Stop Encoder Video Transfer");
    lRet = m_pChnManager->m_pEncSet->SetEncTransfer(&sEncNetSet, iEncChnno, true);
    return lRet;
}
