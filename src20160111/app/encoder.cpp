								 ...
								 int iLayer)
{
	...
	_DEBUG_( "text: %s font:%d x:%d y:%d ", szText, 
		iFont, 
		iStartX, 
		iStartY );
...
HI_S32 CHiEncoder::StartNet( EncNetParam param )
{
	...
	for ( int i = 0; i < MAXNET_COUNT; i++ )
	{
		if ( m_netsetting[i].iEnable == 0 ) continue;
		
		if ( m_netsetting[i].iMux == 2 )
		{
			m_iTsMux = 1;
		}
		if ( m_netsetting[i].iMux == 1 )
		{
			m_iPsMux = 1;
		}
	}
...
HI_S32 CHiEncoder::StartVenc(EncoderParam param)
{
...
HI_S32 CHiEncoder::InitVenc()
{
	...
	_DEBUG_("bind vpss:%d %d", m_iChannelNo, m_iChannelNo /2 );
	if ( m_iChannelNo % 2 == 0 )
	{
...
void CHiEncoder::OnAVReceived( bool isVideo, unsigned char *pBuf, int nLen )
{
	...
	if ( m_iTsMux > 0 )
	{
		...
		while ( m_pEs2Ts->GetTSData(pOutTSData, iOutTSDataLen) ==  0 )
		{
			...
			{
				//if ( m_iChannelNo == 0 )
				//fwrite( pOutTSData, 1, iOutTSDataLen, file );
				for ( int i = 0; i < MAXNET_COUNT; i++ )
				{
					if ( m_netsetting[i].iEnable > 0 && 
