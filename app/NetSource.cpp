...
void CNetSource::ReceivedCB( char *buf, int iSize, char* szIP, int nPort, void* pObj)
{
	...
	if ( iSize > 0 )
	{	
		//_DEBUG_("Recv Data[%d] from %s:%d", iSize, szIP, nPort );
...
void *CNetSource::RecvProc( void *arg )
{
    ...
    int nMaxSize = 2*1024;
	unsigned char *pbuf = (unsigned char*)malloc( 4 * 1024 );
	...
	while( pThis->m_iExit == 0 )
	{
		...
		else
		{
            ...
            if(pThis->m_iExit == 0)
            {
			    ...
			    //usleep( 100000 );
			    CFacility::SleepMsec(50);// 20180201 修改为50ms，暂未发现影响
			}
			else
			{
			    break;
