...
{	
	...
	m_nAudioCounter = 0;
	m_nContinus = 0;
	m_nLastFormat = MPEG2_VIDEO_STREAM_TYPE;
	m_nFormat = MPEG2_VIDEO_STREAM_TYPE;
			...
			&& 0x3E!=*(pbFrameData + i + 4) )
		{
			...
			if ( !bFirstFind && i>0 )
			{
					...
					iLength > 1000000 )
				{
					...
					nRemoveIndex = lFrameSize; // 长度有问题是否返回失败，而不是赋值????20180417
					nRet = 0;
					return nRet;
				}
				lFrameSize -= i;
...
HI_S32 CDecoder::InitVdec()
{
    ...
    /*
    MPP_CHN_S stSrcChnMD;
    MPP_CHN_S stDestChnMD;

    stSrcChnMD.enModId = HI_ID_VDEC;
    stSrcChnMD.s32DevId = 0;
    stSrcChnMD.s32ChnId = VdChn;

    stDestChnMD.enModId = HI_ID_VPSS;
    stDestChnMD.s32DevId = VpssGrp;
    stDestChnMD.s32ChnId = VPSS_PRE1_CHN;

    s32Ret = HI_MPI_SYS_Bind(&stSrcChnMD, &stDestChnMD);
    if (s32Ret != HI_SUCCESS)
    {
    ...
    */
    /*
    s32Ret = SAMPLE_COMM_VENC_BindVpss(0,0, VPSS_PRE1_CHN);
    {
