...
{
    _DEBUG_("start: channel=%d", msg.body.decparam.iSlot);
	//pthread_mutex_lock( &m_lock );
	int nChannel = msg.body.decparam.iSlot;
		...
		NET_MULTICAST == msg.body.decparam.bProtocol )
	{
		...
		_DEBUG_( "######## Start Decoder [UDP] chann:%d url:%s\n", nChannel, szUrl);
	...
	else if ( NET_TCP == msg.body.decparam.bProtocol )
	{
		...
		_DEBUG_( "######## Start Decoder [TCP] chann:%d url:%s", nChannel, szUrl);
    ...
    else if ( NET_URL == msg.body.decparam.bProtocol )
    {
        ...
        _DEBUG_( "######## Start Decoder [URL] chann:%d url:%s", nChannel, szUrl);
		m_pChnManager->SaveUrl(nChannel, szUrl, iFps);
    ...
    {
		_DEBUG_( "######## Stop Decoder [NONE] chann:%d url:%s", nChannel, "");
	...
	else
    {
		...
		_DEBUG_( "######## Start Decoder [OTHER] chann:%d url:%s", nChannel, szUrl);
		m_pChnManager->SaveUrl(nChannel, szUrl, iFps);
		m_pChnManager->Open( nChannel, szUrl, msg.body.decparam.uiStreamType, iFps );
		memcpy( &m_decparam[nChannel-1], &msg.body.decparam , sizeof(COMDECODERPARAM));
    }
	...
	//pthread_mutex_unlock( &m_lock );
	_DEBUG_("end  : channel=%d", msg.body.decparam.iSlot);
	return 1;
}
...
int CMDProtocol::GetMsg( CSyncNetEnd *net, MDMsg &msg, int timeout)
{
	...
	{
		_DEBUG_("Receive Incorrect length=%x", length);
		return -1;
	}
	...
	if ( len!=length )
	{
		_DEBUG_("Receive Length:%d!=Respect_Length:%d", len, length);
		return -1;
	}
