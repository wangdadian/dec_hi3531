...
public:
	...
	// 获取、设置自动重启信息
    static long GetAR(char* szBuf, void* param);
    static long SetAR(const char* szBuf, void* param);
    // OSD
...
private:
    ...
    long load_userinfo();
    static void* ThNtpWorker(void* param); // NTP设置线程
	static void* ThARWorker(void* param);  // 自动重启线程
...
private: 
	...
	struARInfo m_objARInfo;
    pthread_t m_thNtpWorker;
    bool m_bNtpWorkerExit;
    pthread_mutex_t m_ntpMutexLock;
	pthread_t m_thARWorker;
    bool m_bARWorkerExit;
    pthread_mutex_t m_arMutexLock;
